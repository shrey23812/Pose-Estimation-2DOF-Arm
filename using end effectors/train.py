...
for f in listdir(DATA_DIR):
	...
	var = f[1:-5].split(",")
	print(var, " : ", model.predict(img)[0],i,np.linalg.norm(np.array(var,dtype="float32")-model.predict(img)[0]))#,abs(np.array(var,dtype="float32")-np.array(model.predict(img)[0])))
	x.append(np.linalg.norm(np.array(var,dtype="float32")-model.predict(img)[0]))
	y.append(i)
	i = i+1
